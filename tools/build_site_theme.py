...
/* Cada faixa leva o rotulo da sua propria cor de fundo. A primeira e o verde
   de preenchimento, que e claro nos dois temas, entao o rotulo dela e escuro
   sempre. As outras duas seguem o tema, e por isso usam --fg: fixar preto nelas
   dava 1,18 de contraste no escuro. */
.alpha-split span:nth-child(1), .alpha-split span:nth-child(1) * {
  color: var(--acao-vivo-fg) !important; }
.alpha-split span:nth-child(2), .alpha-split span:nth-child(2) *,
.alpha-split span:nth-child(3), .alpha-split span:nth-child(3) * {
...
/* Esperado contra realizado: tres blocos, sem fio entre eles. */
.fc-perfil { margin-top: 34px; }
.fc-perfil h3 { color: var(--acao); margin: 0 0 6px; }
.fc-perfil p { color: var(--fg-2); margin: 0 0 12px; max-width: 68ch; }
.fc-perfil p b { color: var(--fg); }
.fc-fig { margin: 0; }
.fc-fig svg { display: block; width: 100%; height: auto; max-width: 620px; }
.fc-fig figcaption { margin-top: 8px; font-size: 12px; color: var(--fg-2); max-width: 68ch; }
#forecast-2026-note { max-width: 68ch; margin-top: 26px; color: var(--fg-2) !important; }

/* Os fios entre secoes. A pagina tem oito secoes e cada uma vinha com um
   traco em cima: o olho le como grade, nao como divisao. O espaco separa. */
.version-section, .version-section-heading, .version-section > div,
.section-heading, .proof, .method-grid { border-top: 0 !important;
  border-bottom: 0 !important; }
.version-section { margin-top: 64px; }
/* As duas linhas verticais que ladeavam o grafico: sao a borda do container
...
/* Painel branco sobre página branca.
 *
 * No desenho original a página e os painéis eram os dois brancos, e o que
 * separava um do outro era a borda. O gerador, que classifica por luminância,
 * mapeou os dois para --canvas, e enquanto --canvas foi branco ninguém notou.
 *
 * Quando a página virou menta e o cartão virou branco, esses painéis ficaram
 * com a cor exata da página. Sem borda, que também tinha saído, o contêiner
 * desapareceu e o conteúdo passou a flutuar solto. É a mesma lição de sempre:
 * luminância não distingue "página" de "painel", só o DOM sabe qual é qual.
 *
 * A lista abaixo veio de medir a página renderizada. Estes contêm conteúdo,
 * então são cartão, e o degrau de superfície faz o trabalho que a borda fazia. */
.comparison-card, .wealth-panel, .chart-return-summary, .chart-add-details,
.fixed-comparators, .radar-summary, .paper-metric, .version-step,
.benchmark-panel, .comparison-foot, .quick-composition {
  background: var(--card) !important;
  border-radius: 14px !important;
}
/* Os que vivem dentro de um cartão não precisam de superfície própria: dois
   brancos encostados viram um branco só, com uma emenda no meio. */
.return-chart, .chart-sidepanel, .comparison-controls, .chart-tools,
.comparison-card .chart-legend { background: transparent !important; }
...
/* O halo que segue o ponteiro. Menor que o do resumo, porque aqui ele divide a
   tela com conteudo denso: 150px em vez de 260. Fica no fundo de tudo, e por
   isso o conteudo precisa de contexto de empilhamento proprio, senao ele passa
...
/* As tres linhas da comparacao tem de ser a mesma coisa. Duas delas vinham com
   fundo proprio e a terceira nao, e como nao ha padding nem raio o fundo virava
   um retangulo cru atras de duas linhas de tres. Nenhuma tem fundo: elas ja
   estao dentro do painel, e o que separa uma da outra e a barra. */
.hero-performance-bars > div:first-child,
.hero-performance-bars > div:nth-child(2),
.hero-performance-bars > div:nth-child(3) { background: transparent !important; }
