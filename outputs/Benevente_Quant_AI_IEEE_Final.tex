...
With 36 trials, the winner's Sharpe is biased upward by the search itself. The observed excess-return Sharpe is 0.933 against an expected maximum under the null of 0.355, giving a \textbf{deflated-Sharpe probability of 0.986}, above the 95\% threshold.
...
The drawdown is not incidental. The nested protocol raised the equity budget to 95\% between 2018 and 2021, and the portfolio took essentially the same fall as the index. This is a concentrated book that recovered faster, not a smoothed version of the market. We refer to this canonical annual strategy as Benevente~1.
...
The evaluation window is also the development window; eleven evaluated years are not eleven years of prospective validation, and the 0.65 pp hindsight premium bounds that problem without eliminating it. The corporate-action detector's 23.3\% recall means distributions and events on delisted names still require reconciliation against primary records before any commercial performance claim. Issuers without provider coverage receive the annual cross-sectional median distribution yield, an approximation whose affected names are listed with the panel. Eleven annual observations are few; the deflated-Sharpe probability corrects the search bias, not the sample size. The language-model experiment uses one model at one training cutoff, and separating contamination from capability requires repeating it across models with different cutoffs. The annual cadence fits the accounting signal and beat quarterly and monthly reselection in this sample, but it is not claimed to be universally optimal. The Benevente~2 extension is retrospective, was designed after Covid-19, and omits intrayear tax; it must be pre-registered and evaluated prospectively before promotion. A separate study should test timestamped news and event-driven or quarterly decisions without contaminating the annual protocol. The factor transformation, eligibility thresholds, configuration grid and annual choices are disclosed in the supplementary artefacts; the source code is not required to understand those transformations.
...
\bibitem{b3cotahist} B3 S.A. --- Brasil, Bolsa, Balc\~ao, ``Historical quotations,'' 2026. [Online]. Available: \url{https://www.b3.com.br/pt_br/market-data-e-indices/servicos-de-dados/market-data/historico/mercado-a-vista/cotacoes-historicas/}
...
\bibitem{openai} OpenAI, ``GPT-5 System Card,'' 2025. [Online]. Available: \url{https://openai.com/index/gpt-5-system-card/}
\end{thebibliography}
\end{document}
