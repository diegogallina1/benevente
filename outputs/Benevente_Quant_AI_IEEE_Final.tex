...
\begin{abstract}
Language models are increasingly proposed as portfolio allocators, although historical prompts may reveal outcomes already embedded in model training. We present an auditable architecture in which a dated deterministic rule selects and weights assets, a language model only explains approved facts, and a human records approval. A separate retrospective stress test relaxes that boundary in four arms over thirteen years: named issuers, anonymised issuers, a deterministic factor control, and direct model-generated weights. The measured differences are indistinguishable from zero: named versus anonymised $+0.38$ pp/yr ($p=0.91$), anonymised versus deterministic $-0.05$ pp/yr ($p=0.99$), and solver-routed versus direct weights $+0.81$ pp/yr ($p=0.78$). Anonymisation cannot prove temporal purity because financial attributes may still identify firms and pretrained memory is unobservable. The direct-weight arm failed to sum to one in five of thirteen years and omitted eligible names in two. The evaluation infrastructure contains 514 issuers, including 166 that stop trading, receipt-date gating, nested selection with a 0.65 pp/yr hindsight premium, and a deflated-Sharpe probability of 0.986 over 36 trials. Seven audited defects had inflated earlier results, and a primary-event reconciliation gate also failed. Accordingly, the historical evidence diagnoses the measurement system rather than validating language-model alpha; scientific evaluation of the language component begins with a frozen prospective protocol. A subsequent controlled experiment on the deterministic layer sharpened this stance: widening the nested configuration search from 36 to 256 candidates, on identical inputs, cost 2.63 percentage points a year of realised return and collapsed the deflated Sharpe from 0.957 to 0.777, so the production rule is now declared and frozen per investor profile rather than searched. If even a deterministic selector saturates at tens of candidates on ten annual observations, delegating selection to a language model is not a capability question but a capacity one.
\end{abstract}
...
We therefore fix a production architecture in which the model cannot select an asset or set a weight. It receives a completed decision record and produces only explanation, risks and questions for human review. A future extension, called Benevente 3 in the product roadmap, will evaluate explanation fidelity, completeness, risk coverage and fabricated numbers prospectively; it is not part of the return claims in this paper. A separate retrospective diagnostic temporarily relaxes the production contract to measure three comparisons that are usually conflated:
...
Delisted tickers are not served by a conventional price provider. The initial round-ratio detector had 88.1\% precision but only 23.3\% recall against the available event archive. We therefore queried the current primary B3 corporate-action page for 497 price-panel securities \cite{b3events}. It responded for 475. After removing records predating each security's observed span, the archive contains 9,772 events, including 127 subscriptions and 153 events requiring manual treatment. Endpoint response is not historical completeness. We tested the archive on the 56 equity holding-years actually used by the published strategy: 54 could be compared, seven differed from the published adjusted return by more than five percentage points, and two MRFG3 observations had no endpoint response. No manual event overlapped an actual holding. The reconciliation gate therefore failed and the performance panel remains research-grade.
...
The protocol of record for the period reported here is nested. For decision year $t$, the 36 configurations are ranked using only years that closed before $t$, by Sharpe of the excess return over cash; year $t$ is then evaluated once. Changing configuration is charged a full-turnover rebalance. Ranking requires three closed years, so 2012--2014 is the selection window and 2015--2025 is the evaluation window, giving eleven decisions.
...
\emph{Posterior result (August 2026).} The nested defence was then stress-tested on itself: with inputs, code and window held identical (the 36 shared configurations reproduce across runs to $5.6\times10^{-17}$), the same procedure over 256 candidates realised 12.68\% a year against 15.31\% for the 36-candidate grid, and the deflated Sharpe fell from 0.957 to 0.777 --- below significance --- because the expected maximum under the null rose from 0.375 to 0.746. Nested selection therefore has a \emph{capacity limit} in the number of candidates its ranking step can support; past it, the guarantee degrades silently. The production rule is now \emph{declared}: one frozen configuration per investor profile, registered with input hashes, a named approver, a falsification criterion, and a confirmatory sample starting with the first trading session of 2027. A companion manuscript documents the experiment and the declared policy in full; every published figure is machine-checked against the versioned artifacts.

\emph{Second posterior result (August 2026).} The declared policy itself was then revised once, for a reason that belongs in the record. Its cash sleeve was an index --- 100\% of the CDI compounded daily --- and an index carries no custody fee, no bid--ask spread and no roll. Version~3 replaces it with the instrument an investor actually buys, a Brazilian Treasury floating-rate note rebuilt from the sovereign's own daily file and net of the exchange's custody schedule and of the spread charged at each roll. Nothing else changes, which is what makes the effect attributable. Cash yields 9.36\% a year against 9.68\% for the index, every profile's return falls by 0.07 to 0.17 points, and the excess over cash \emph{rises} by 0.08 to 0.18, because the benchmark is entirely cash and the portfolio is not. The swap also exposed a data defect: the previous panel's cash column is constant until 2012, so the nested selector ranked configurations with cash earning zero in that year, penalising the more defensive alternatives. Both arms of the capacity experiment used the same input, so the comparison between them stands; what cannot be claimed is that the ordering would have been identical with correct cash.
...
The supplementary product record uses the Ibovespa total-return index, which reinvests distributions in a theoretical portfolio, as the single domestic market reference \cite{b3ibov}. Benevente 1 remains the published annual rule. Benevente 2 is the risk-overlay extension monitored as a shadow portfolio since 2026: it preserves Benevente 1's annual basket and reduces equity exposure after observable market stress. Retrospectively, the training-only configuration reduced maximum drawdown from 47.8\% to 28.7\% and raised CAGR from 17.86\% to 18.45\%, although the 2019--2025 annual return difference was not detectable ($p=0.964$). For BRL~100,000, an aggregate 15\% tax estimate for intrayear risk reductions was BRL~2,195 and reduced terminal wealth from BRL~643,774 to BRL~634,531. This is not tax-lot reconciliation or prospective validation. A Gemini news radar now classifies current alerts for human review, but neither alters weights nor enters the historical experiment.
...
The evaluation window is also the development window; the 0.65 pp hindsight premium bounds that problem without making the record prospective. The current B3 endpoint responded for 95.6\% of price-panel securities, but its archive failed to reproduce seven of 54 compared holding-year returns within five percentage points and omitted two further observations. Issuers without provider coverage receive an explicitly listed cross-sectional median distribution yield. Eleven annual observations remain few: the deflated-Sharpe probability corrects search bias, not sample size. The model experiment covers one model and one training cutoff. The annual cadence follows from the fundamental focus rather than from a general result: it beat quarterly and monthly reselection here, but is not universally optimal, and a shorter horizon would require a different signal base. Benevente~2 was designed after Covid-19 and uses an aggregate, capital-sensitive intrayear tax estimate rather than issuer-level tax-lot reconciliation; it therefore still requires pre-registration and prospective evaluation. Timestamped news and event-driven decisions belong in a separate protocol. Factor transformations, thresholds, the configuration grid and annual choices are disclosed in the supplementary artefacts.

The 36-trial deflated-Sharpe figure quoted above certifies the archived run, not the procedure's robustness to widening: at 256 trials the same statistic falls below significance, which is why the live rule is declared rather than searched.
...
\bibitem{b3events} B3 S.A. --- Brasil, Bolsa, Balc\~ao, ``Dividends and other corporate actions,'' 2026, \href{https://sistemaswebb3-listados.b3.com.br/dividensOtherCorpActPage/}{online}.
