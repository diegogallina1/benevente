...
2. Gerar fundamentos a partir de DFP oficial da CVM, usando o mapa contábil específico para companhias não financeiras ou instituições financeiras, e verificar a data contábil (`as_of_date`) e a data de divulgação (`available_date`).
...
- ITR trimestral da CVM: <https://dados.cvm.gov.br/dados/cia_aberta/doc/itr/DADOS/>. A próxima evolução deve substituir o DFP anual pelo ITR mais recente quando este estiver publicamente disponível.
